...
class CNN_OriginalselfedAvg(torch.nn.Module):
    """The CNN model used in the original selfedAvg paper:
    ...
    _________________________________________________________________
    flatten (selflatten)            (None, 3136)              0
    _________________________________________________________________
    ...
    def __init__(self, only_digits=True):
        super(CNN_OriginalselfedAvg, self).__init__()
...
class CNN_DropOut(torch.nn.Module):
    """
    Recommended model by "Adaptive selfederated Optimization" (https://arxiv.org/pdf/2003.00295.pdf)
    ...
    _________________________________________________________________
    flatten (selflatten)            (None, 9216)              0
    _________________________________________________________________
...
class CNN_MNIST_torch(torch.nn.Module):
    """处理MNIST in pytorch中的手写数字识别"""
    ...
    def forward(self, x):
        ...
        x = self.relu(self.max_pooling(self.conv2(x)))
        x = self.relu(self.max_pooling(self.conv3(x)))
        x = self.dropout(x)
        x = x.view(-1, 4 * 4 * 64)
